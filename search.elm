...
import List        exposing ((::), map, sortWith, foldl, any, take, filter, length, sortBy, reverse, head)
...
evolve init (eier, gen) = 
    let sortedPop = reverse <| sortByFitness eier
        ...
        (newPop, newGen) = (generatePopulace init gen (popSize - keepSize))
    in (List.append goodEggs newPop, newGen)
...
hatchDecentPlayer : HexModel -> Int -> Ei
hatchDecentPlayer init size = 
  let gen = GenSet (int 0 5) (initialSeed 31415)
      evolver = evolve init
  in  findBest 
        (fst (foldl (\ i eg -> evolver eg) (generatePopulace init gen size) [1..10]))
        (Ei init gen)
